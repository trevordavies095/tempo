...
### Added
- **FIT session RPE → Workout** - New FIT imports (intervals.icu sync, command-center FIT, Strava-bulk original FIT) fill `Workout.Rpe` (1–10) from session `workout_rpe` (Borg CR10 × 10) when the column is null. Existing rows are unchanged; Feel, Garmin Connect-only ratings, and intervals.icu `icu_rpe` are not sources. Crop leaves RPE alone.
- **Named logging profiles (`standard` | `debug`)** - `Tempo:Logging:Profile` / `Tempo__Logging__Profile` (Compose `.env`: `TEMPO_LOGGING_PROFILE`, default `standard`). Startup logs `Logging profile: …`. `standard` keeps Tempo Information, quiets EF SQL and per-request spam (5xx only); `debug` restores SQL and non-probe request traces (not Serilog Debug). Successful `/health` and `/ready` request lines stay quiet. Unknown profile fails startup. Official Compose Postgres uses `log_checkpoints=off` (not driven by the API profile).
...
- **Production Compose publish surface** - command center on `127.0.0.1:3004` only; Postgres and API unpublished by default (opt-in commented `127.0.0.1:5001`). CORS / LAN origins removed from the file. One reverse-proxy upstream; merge prior `/api`→`:5001` splits or uncomment loopback API. Backups use `docker compose … exec postgres` and `$POSTGRES_PASSWORD` (not `tempo-postgres` container names).
...
- **Startup FIT device-lap backfill** - `DeviceLapBackfillWorker` / `DeviceLapBackfillService` copies FIT laps into `device_lap` for existing workouts (JSON `laps` or reparsed file bytes). Idempotent via top-level `lapsBackfill` markers (`applied` | `skipped_single` | `skipped_empty` | `skipped_crop` | `absent`); skips when session elapsed differs from `DurationS` by more than 1s (cropped). Does not rewrite distance splits or geometry.
- **HealthKit optional laps** - schema v1 additive `laps` (`endedAt`, `distanceM`, `durationS` timer, optional `avgHeartRateBpm`); same keep/2+ persist as FIT. Repeat UUID POST attaches laps (`updated`) when the Workout has none yet, without rewriting geometry; otherwise still `skipped`.
