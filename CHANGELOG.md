...
### Fixed
- **FIT cadence as steps/min** - FIT record and session avg/max cadence are stored as steps/min (both feet; ×2 from FIT strides/min). Workout overview labels cadence **spm**. API JSON keys stay `cadenceRpm` / `avgCadenceRpm` / `maxCadenceRpm`; the number is steps/min. GPX and HealthKit cadence are unchanged.
