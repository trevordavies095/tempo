...
- **Unit Tests**: Test individual services in isolation (located in `api/Tempo.Api.Tests/Services/`)
  ...
  - `WorkoutIntakeTests` cover `created` / `updated` / `skipped` / `error` with faked weather, relative effort, and best efforts; persist and duplicate stay real. Includes a `PersistAsync` seam test that builds `DecodedWorkout` without the file adapter, plus HealthKit outdoor persist/skip cases (UUID identity, different-stats same UUID, GPX stamp). `HealthKitWorkoutDecoderTests` and `ImportHealthKitWorkoutTests` cover the JSON import path, including parallel same-UUID posts.
