# Developer Documentation

Welcome to the Tempo Developer Documentation! This section provides technical information for developers who want to understand, contribute to, or extend Tempo.

## Overview

Tempo is a full-stack application built with modern web technologies. This documentation covers architecture, development setup, API reference, database schema, and contributing guidelines.

## Topics Covered

- **[Architecture](architecture.md)** - System architecture, components, and design patterns
- **[Development Setup](setup.md)** - Local development environment setup
- **[API Reference](api-reference.md)** - Complete API endpoint documentation
- **[Database Schema](database.md)** - Database structure, relationships, and migrations
- **[Contributing](contributing.md)** - How to contribute to the project

## Technology Stack

- **Frontend**: Next.js 16, React 19, TypeScript, Tailwind CSS (semantic tokens, Geist, hand-rolled UI kit)
- **Icons**: Tabler Icons (@tabler/icons-react)
- **Backend**: ASP.NET Core 9 Minimal APIs (C#)
- **Database**: PostgreSQL 16 with JSONB fields
- **State Management**: TanStack Query
- **Maps**: Leaflet/React-Leaflet
- **Charts**: Recharts

## Quick Links

- [GitHub Repository](https://github.com/trevordavies095/tempo)
- [Issue Tracker](https://github.com/trevordavies095/tempo/issues)
- [Discord Community](https://discord.gg/9Svd99npyj)

## Getting Started

If you're new to Tempo development:

1. Read the [Architecture Overview](architecture.md) to understand the system
2. Set up your [Development Environment](setup.md)
3. Explore the [API Reference](api-reference.md) to understand endpoints
4. Review the [Database Schema](database.md) to understand data models
5. Check the [Contributing Guide](contributing.md) before submitting changes

## Development Workflow

1. Fork the repository
2. Create a feature branch
3. Make your changes
4. Test thoroughly
5. Submit a pull request

For detailed guidelines, see the [Contributing Guide](contributing.md).

